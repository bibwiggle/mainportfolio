...
//       style={{
//         transform: `translate(-100%, -100%) translateY(${parallaxOffset}%)`,
//         left: '100%',
//         top: '100%',
//       }}
//     >
...
export function BackgroundAnimation() {
  ...
  // Scroll handling with improved scroll range calculation
  const handleScroll = useCallback(() => {
    const scrollPosition = window.scrollY;
    const pageHeight = document.documentElement.scrollHeight;
    const windowHeight = window.innerHeight;
    ...
    const extendedScrollRange = scrollRange / scalor;
    const progress = scrollPosition / extendedScrollRange;
    targetScrollProgressRef.current = progress;
  }, []);
  ...
  const smoothScrollAnimation = useCallback(() => {
    setScrollProgress(prev => {
      ...
      return prev + diff * 0.15; // Adjust smoothing speed
    });
    animationFrameRef.current = requestAnimationFrame(smoothScrollAnimation);
  }, []);
  ...
  const updateSize = useCallback(() => {
    if (containerRef.current) {
      const aspectRatio = 1; // Match your animation's aspect ratio
      const windowWidth = window.innerWidth;
      const windowHeight = window.innerHeight;
      const windowRatio = windowWidth / windowHeight;
      ...
      if (windowRatio > aspectRatio) {
        // Window wider than animation
        height = windowHeight;
        width = height * aspectRatio;
      } else {
        // Window taller than animation
        width = windowWidth;
        height = width / aspectRatio;
      }

      // Direct DOM manipulation for immediate update
      containerRef.current.style.width = `${width}px`;
      containerRef.current.style.height = `${height}px`;
